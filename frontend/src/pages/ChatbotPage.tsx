    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
          ...
          <div className="p-3 border-b border-cyber-800 space-y-2">
            <div className="relative">
              ...
              <input
                ...
                className="chat-input w-full pl-8 pr-3 py-1.5 text-xs rounded-lg bg-cyber-950/80 border border-cyber-750 text-cyber-100 placeholder-cyber-500 focus:border-shield-cyan focus:outline-none"
              />
            </div>
          </div>
                ...
                return (
                  <div
                    ...
                    className={`rounded-xl border transition ${
                      isActive
                        ? 'bg-shield-cyan/10 border-shield-cyan/40'
                        : 'border-transparent hover:bg-cyber-850 text-cyber-300'
                    }`}
                  >
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <p className="mt-1.5 text-[10px] text-cyber-500 font-mono">{formatShortDate(conversation.updated_at)}</p>
                    </button>
                  </div>
                );
              })
        ...
        <section className="cyber-card flex flex-col h-[78vh] overflow-hidden">
          <div className="px-5 py-3.5 border-b border-cyber-800 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <h2 className="text-sm font-semibold text-white truncate">
                {activeConversation?.title || 'New conversation'}
              </h2>
              <p className="text-xs text-cyber-400 truncate">Company Knowledge Assistant • Autonomous Semantic Security</p>
            </div>
            {activeConversation && (
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
            {!activeConversation || activeConversation.messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full max-w-3xl mx-auto text-center space-y-6 py-8 animate-fadeIn">
                ...
                <div className="w-full grid grid-cols-1 gap-3 text-left md:grid-cols-2 xl:grid-cols-3">
                  {SUGGESTED_PROMPTS.map((promptText) => (
                    <button
                      key={promptText}
                      type="button"
                      onClick={() => handleSend(promptText)}
                      className="w-full rounded-2xl border border-cyber-800 bg-cyber-900/80 px-4 py-3 text-left text-sm text-cyber-200 transition hover:border-shield-cyan/50 hover:bg-cyber-850 hover:text-white"
                    >
                      {promptText}
                    </button>
                  ))}
              ...
              activeConversation.messages.map((message) => {
                ...
                return (
                  <div
                    ...
                    className={`flex ${isUser ? 'justify-end' : 'justify-start'} group animate-fadeIn scroll-mt-6`}
                  >
                    <div
                            ...
                            <div className={`mb-4 rounded-2xl border px-4 py-3 ${riskTone.container}`}>
                              <div className="flex items-start gap-3">
                                ...
                                <div className="space-y-1">
                                  <p className="text-sm font-semibold">{isBlock ? 'High-risk protected response withheld' : riskTone.label}</p>
          ...
          <div className="p-4 border-t border-cyber-800 bg-cyber-950/50">
            <div className="rounded-2xl bg-cyber-900 border border-cyber-750 p-2.5 focus-within:border-shield-cyan transition shadow-inner">
              <textarea
                ...
                disabled={isSending}
                className="chat-input w-full resize-none bg-transparent px-2 text-sm text-cyber-100 placeholder-cyber-500 focus:outline-none leading-relaxed"
              />
