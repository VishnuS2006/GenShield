...
async def dashboard(_: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> DashboardResponse:
    ...
    recent = list(
        await db.scalars(select(DetectionResult).order_by(DetectionResult.created_at.desc()).limit(10))
    )
